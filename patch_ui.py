...
# We want to replace the block starting from 'local mapsConfig =' down to '-- Thêm UI Auto Refresh'
...
new_block = """        local mapsConfig = {
            {
                ...
                bosses = {
                    { id = 10179307, name = "Kỵ Sĩ(T)", col = 1, transferId = 400213, posX = 154, posY = 113 },
                    { id = 10179307, name = "Kỵ Sĩ(D)", col = 1, transferId = 400213, posX = 145, posY = 84 },
            ...
            },
            {
                ...
                bosses = {
                    { id = 10640201, name = "Cây Totem", col = 1, transferId = 10640201, posX = 100, posY = 146 },
                    { id = 10640202, name = "Ngang Ngược", col = 2, transferId = 10640202, posX = 68, posY = 84 },
                    { id = 10640203, name = "Tà Ác KL", col = 3, transferId = 10640203, posX = 206, posY = 152 },
                }
            }
        }
        ...
        local function UpdateBossWatchUIText()
            pcall(function()
                    ...
                    for r = 1, maxRows do
                        for c = 1, 3 do
                            ...
                            if cfg then
                                ...
                                local uiBtn = GetLineButton(btnIdx, startX + 110, yPos, 110)
                                ...
                                if bossData then
                                    ...
                                    for _, lineNum in ipairs(bossData.lineNums) do
                                        local ca = consumedAlive[cfg.id] and consumedAlive[cfg.id][lineNum] or 0
                                        local cd = consumedDead[cfg.id] and consumedDead[cfg.id][lineNum] or 0
                                        
                                        local totalAlive = bossData.aliveCount[lineNum] or 0
                                        local deadList = bossData.deadTimes[lineNum] or {}
                                        
                                        if ca < totalAlive then
                                            statusStr = "<color=#00FF00>xuất hiện</color>"
                                            if not consumedAlive[cfg.id] then consumedAlive[cfg.id] = {} end
                                            consumedAlive[cfg.id][lineNum] = ca + 1
                                            bestLine = lineNum
                                            break
                                        elseif cd < #deadList then
                                            local rt = deadList[cd + 1]
                                            ...
                                            if not consumedDead[cfg.id] then consumedDead[cfg.id] = {} end
                                            consumedDead[cfg.id][lineNum] = cd + 1
                                            bestLine = lineNum
                                            break
                                        end
                                    end
                ...
                panelRt.sizeDelta = Vector2(700, requiredHeight)
            end)
        end
            ...
            _G.SceneData.MonsterMapDataInit = function(data)
                ...
                pcall(function()
                    if _G.SavedFOV then
                        ...
                        if cam then cam.fieldOfView = _G.SavedFOV end
                    end
                end)
            end
