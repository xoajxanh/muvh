local Activity_CommercialTimeLimited_Page = {}
Activity_CommercialTimeLimited_Page.activityData = nil

function Activity_CommercialTimeLimited_Page:Init()
  self:InitControls()
end

function Activity_CommercialTimeLimited_Page:InitControls()
...
function Activity_CommercialTimeLimited_Page:Refresh(data, ui)
  self.lab_name:SetText(data.commerceName)
  self.img_clickeffect:SetActive(data.Selected)
  self.img_redPoint:SetActive(RedPointChecker_Ext:GetTogTimeLimitedRedPoint(CommerceTimeLimitedRedTogType[data.group]))
  self:UIControl().data = data
  self:UIControl():SetOnClick(ui, ui.BtnHolidayOnClick)
  self.activityData = gameMgr:GetGlobalActivityDataManager():GetActivityManger(data.commerceType):GetActivityData(data.group)
  self.basePanel = ui
  self:RefreshCtrName()
  self:BindRedPoint()
end

function Activity_CommercialTimeLimited_Page:RefreshCtrName()
  if self.activityData and self.activityData:GetRedPointTbl() ~= nil then
    self:UIControl().gameObject.name = self.activityData:GetRedPointTbl().IdEnum
  end
end

function Activity_CommercialTimeLimited_Page:GetRedPointPath()
  return self.basePanel.root.gameObject.name .. "#" .. self:UIControl().gameObject.name
end

function Activity_CommercialTimeLimited_Page:SetClickEffect(isActive)
  self.img_clickeffect:SetActive(isActive)
end

function Activity_CommercialTimeLimited_Page:BindRedPoint()
...
function Activity_CommercialTimeLimited_Page:RemoveCacheRedPoint(checkSameRedPoint)
...
function Activity_CommercialTimeLimited_Page:Exit()
  self:RemoveCacheRedPoint()
end

return Activity_CommercialTimeLimited_Page
