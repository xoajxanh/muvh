...
function Mathf.RandomTableValue(t)
  math.randomseed(os.time())
  local random = Mathf.Random(1, #t)
  return t[random]
end

function Mathf.NumberShowFormat(number, sep, item3D)
  if number then
    number = tonumber(number)
    if number / 1000 >= 1 then
      local divided = number / 1000
      if divided % 1 == 0 then
        return string.format("%dK", divided)
      else
        return string.format("%sK", divided)
      end
    end
    return number
  end
...
function Mathf.keepthreefloor(decimal)
  decimal = decimal * 1000
  decimal = math.floor(decimal)
  return decimal * 0.001
end

function Mathf.keeptwofloor(decimal)
  decimal = decimal * 100
  decimal = math.floor(decimal)
  return decimal * 0.01
end
