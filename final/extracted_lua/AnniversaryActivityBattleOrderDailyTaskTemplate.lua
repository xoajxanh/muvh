local AnniversaryActivityBattleOrderDailyTaskTemplate = {}

function AnniversaryActivityBattleOrderDailyTaskTemplate:Init()
  self:InitControls()
  self:RegistUIEvents()
end

function AnniversaryActivityBattleOrderDailyTaskTemplate:InitControls()
...
function AnniversaryActivityBattleOrderDailyTaskTemplate:RegistUIEvents()
  self.lab_CanNotGet:SetOnClick(self, self.lab_CanNotGetOnClick)
  self.btn_get:SetOnClick(self, self.btn_getOnClick)
end

function AnniversaryActivityBattleOrderDailyTaskTemplate:lab_CanNotGetOnClick()
  AnniversaryActivityController.GoTask(self.data)
end

function AnniversaryActivityBattleOrderDailyTaskTemplate:btn_getOnClick(control)
end

function AnniversaryActivityBattleOrderDailyTaskTemplate:Refresh(data, ui)
  self.data = data
...
function AnniversaryActivityBattleOrderDailyTaskTemplate:SetTaskState(state)
  ...
  if state == TaskStateEnum.CanNotGet then
    local str = ClientTable.cfg_Ui_wordManager:TryGetValue("ZhouNianQing_task").content
    self.lab_taskProgress:SetText(string.format(str, self.goalInfo.current, self.goalInfo.count))
  end
end

return AnniversaryActivityBattleOrderDailyTaskTemplate
