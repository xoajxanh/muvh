...
function Activity_Sport3V3Score:btn_closeBgOnClick(control)
  if self.resultData and self.resultData.competition then
    networkRequest.ReqGetMatchTeamInfo(0, ReqTeamInfoType.Default)
    local tipArgs = QuickFind:GetTeam3V3DataMgr():GetDelayTipsInfo()
    if not table.isNullOrEmpty(tipArgs) then
...
function Activity_Sport3V3Score:Refresh(_, data)
  self.resultData = data or self.args and self.args.resultData
  if table.isNullOrEmpty(self.resultData) then
    local MainInfo
    ...
    local myCampPlayerInfo = self.resultData.myResultScoreCampInfo and self.resultData.myResultScoreCampInfo:GetPlayerInfoList()
    local enemyCampPlayerInfo = self.resultData.enemyResultScoreCampInfoList[1] and self.resultData.enemyResultScoreCampInfoList[1]:GetPlayerInfoList()
    local redCamp, blueCamp
    ...
    local isWin = self.resultData.myResultScoreCampInfo and self.resultData.myResultScoreCampInfo.GroupId == self.resultData.winGroupType
    self:SetSprite("Atlas_Language", isWin and "3V3Win" or "3V3Fail", self.resultTitle)
  end
  local isResultRefresh = self.resultData ~= nil
