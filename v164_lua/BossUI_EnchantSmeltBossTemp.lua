local BossUI_EnchantSmeltBossTemp = {}

function BossUI_EnchantSmeltBossTemp:Init()
  self:InitControl()
  self:InitContainer()
end

function BossUI_EnchantSmeltBossTemp:InitControl()
...
local function OnMapItemCreate(control)
  ...
  control.lab_id = UIControl(control.transform, "lab_id")
end

function BossUI_EnchantSmeltBossTemp:InitContainer()
  self.mapItemTemp = UIContainer(self.mapItem, self, OnMapItemCreate)
  self.bossMapTab = {}
end

function BossUI_EnchantSmeltBossTemp:SetDefaultSelectedBossIndex()
  local bossData = MonsterData.FilterSameBossId(MonsterData.GetMonsterBossType(MonsterBossType.EnchantSmeltBoss, true))
  self.bossDataList = {}
...
function BossUI_EnchantSmeltBossTemp:RefreshBossTableView()
...
function BossUI_EnchantSmeltBossTemp:UpdateWildCellCallBack(index)
...
function BossUI_EnchantSmeltBossTemp:RefreshLeftView(bossConfig, obj, index)
  ...
  if LoginData.externalNet == false and obj.lab_id and IsNil(obj.lab_id) then
    obj.lab_id:SetText("\230\128\170\231\137\169id\239\188\154" .. monsterConfig.id)
  end
end

function BossUI_EnchantSmeltBossTemp:RefreshRightView(obj, bossConfig)
  ...
  local monstername = ClientTable.cfg_Ui_wordManager:TryGetValue("RongLianHundunMonsterName").content
  self.sl_BossName:SetText(string.format("%s", monstername))
  ...
  if isSatisfy then
    ...
    self.text_Progress:SetText(string.format("%d/%d", bossConfig.smeltNumberLimit, bossConfig.smeltNumberLimit))
  ...
  elseif isSatisfy == false then
    self.text_Progress:SetText(string.format("%d/%d", holySkeletonBossInfo.count, bossConfig.smeltNumberLimit))
    self.fillArea:SetFillAmount(holySkeletonBossInfo.count / bossConfig.smeltNumberLimit)
  else
    self.text_Progress:SetText(string.format("%d/%d", 0, bossConfig.smeltNumberLimit))
    self.fillArea:SetFillAmount(0)
  end
end

function BossUI_EnchantSmeltBossTemp:RefreshShBossReward(bossConfig)
...
function BossUI_EnchantSmeltBossTemp:SetMapOnClick(mapID)
...
function BossUI_EnchantSmeltBossTemp:Refresh(baseUI)
  if baseUI ~= nil then
    self.baseUI = baseUI
  end
...
return BossUI_EnchantSmeltBossTemp
