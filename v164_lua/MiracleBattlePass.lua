local MiracleBattlePass = {}
setmetatable(MiracleBattlePass, LuaClass.CommerceActivity)
MiracleBattlePass.level = 0
MiracleBattlePass.curExp = 0
MiracleBattlePass.nextExp = 0
MiracleBattlePass.levelRewardInfoList = nil
MiracleBattlePass.rareRewardList = nil
MiracleBattlePass.cacheRareIndex = 0
MiracleBattlePass.hasTypeDic = nil
MiracleBattlePass.hasRewardIdDic = nil
MiracleBattlePass.canGetRewardList = nil
MiracleBattlePass.finishTaskInfoList = nil
MiracleBattlePass.notFinishTaskInfoList = nil

function MiracleBattlePass:RefreshDataByServerData(serverData)
  if serverData ~= nil then
    self:RefreshLevelData(serverData)
    self:RefreshRewardData(serverData)
    self:RefreshTaskData(serverData)
    EventManager.Dispatch(Event.CallRefreshRedPoint, {
      id = ERedPointId.combineActivity_zhanlin
    })
    EventManager.Dispatch(Event.CombineWarOrderPassRefesh)
  end
end

function MiracleBattlePass:RefreshLevelData(serverData)
...
function MiracleBattlePass:RefreshRewardData(serverData)
...
function MiracleBattlePass:RefreshTaskData(serverData)
...
function MiracleBattlePass:TryRefreshTaskInfo(taskInfoList, goalInfo)
...
function MiracleBattlePass:Init()
end

function MiracleBattlePass:InitTaskInfoList()
  if self.notFinishTaskInfoList == nil then
...
function MiracleBattlePass:IsNotGetState(level, type)
...
function MiracleBattlePass:IsGotState(id)
...
function MiracleBattlePass:IsHasType(type)
...
function MiracleBattlePass:AddCanGetReward(id)
...
function MiracleBattlePass:RefreshRareRewardListByIndex()
...
function MiracleBattlePass:CalculateRareRewardListDistance(cellSizeX, spaceX, posX, targetPosX)
...
function MiracleBattlePass:RefreshShowRareRewardIndex()
...
function MiracleBattlePass:AnalyseRewardStr(str)
  local rewardsInfo = {}
...
function MiracleBattlePass:AnalyseRewardState(tbl)
...
function MiracleBattlePass:GetLevelRewardInfoList()
...
function MiracleBattlePass:GetLevel()
  return self.level
end

function MiracleBattlePass:GetCurExp()
  return self.curExp
end

function MiracleBattlePass:GetNextExp()
  return self.nextExp
end

function MiracleBattlePass:GetTaskInfoList()
  local taskInfoList = {}
  if self.notFinishTaskInfoList == nil then
    self:InitTaskInfoList()
  end
...
function MiracleBattlePass:GetCanGetRewardList()
  if self.canGetRewardList == nil then
    self.canGetRewardList = {}
  end
...
function MiracleBattlePass:GetShowRareReward()
  return self.levelRewardInfoList[self.cacheRareIndex]
end

function MiracleBattlePass:GetStartIndex()
  local index = 1
...
function MiracleBattlePass:GetCommerceId()
...
function MiracleBattlePass:CheckRedPointState()
...
function MiracleBattlePass:NewTaskInfo(tbl)
  local taskInfo = {}
  if tbl then
    taskInfo.id = tbl.id
...
function MiracleBattlePass:NewTypeRewardInfo(tbl)
...
function MiracleBattlePass:NewLevelRewardInfo(level, isRare, levelReward)
...
function MiracleBattlePass:ResetActivityData()
  self.level = 0
  self.curExp = 0
  self.nextExp = 0
  self.cacheRareIndex = 0
...
function MiracleBattlePass:ResetTaskInfoList()
...
function MiracleBattlePass:ResetCanGetRewardList()
...
function MiracleBattlePass:ResetLevelRewardInfoList()
...
function MiracleBattlePass:ResetRareRewardList()
