...
function Vector3:One()
  if self.oneVector == nil then
    self.oneVector = Vector3.New(1, 1, 1)
  end
  return self.oneVector
end

function Vector3:CircleRotation_UI()
  if self._circleRotation_UI == nil then
    self._circleRotation_UI = Vector3.New(-25, 0, 0)
  end
  return self._circleRotation_UI
end

function Vector3:CircleScale_UI()
  if self._circleScale_UI == nil then
    self._circleScale_UI = Vector3.New(1.3, 1.3, 1.3)
  end
  return self._circleScale_UI
end

function Vector3.NewByString(positionStr)
  ...
  local z = string.isNullOrEmpty(positionList[3]) == false and tonumber(positionList[3]) or 0
...
function Vector3.MoveTowards(current, target, maxDistanceDelta)
  local pos = Vector3()
  Vector3.MoveTowardsNonAlloc(current, target, maxDistanceDelta, pos)
  return pos
end
...
function Vector3.EqualsValue(vector, x, y, z)
  return vector ~= nil and vector.x == x and vector.y == y and vector.z == z
end
...
function Vector3.GetTemp(x, y, z)
  tempV3:Set(x, y, z)
  return tempV3
end
...
function Vector3.Alloc(x, y, z)
  local v3 = table.remove(pool)
  if v3 then
    v3:Set(x, y, z)
  else
    v3 = Vector3(x, y, z)
...
function Vector3.AllocFrom(v3)
  return Vector3.Alloc(v3.x, v3.y, v3.z)
end

function Vector3.Recycle(v3)
  if v3 and v3.recycle then
    v3.recycle = false
    table.insert(pool, v3)
  end
end

function Vector3.RecycleArray(v3s)
  if v3s then
    for k, v in ipairs(v3s) do
      Vector3.Recycle(v)
    end
  end
end
