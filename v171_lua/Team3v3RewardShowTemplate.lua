...
function Team3v3RewardShowTemplate:CreateCharacterModel(index, data)
  local careerSuitDic = QuickFind:GetTeam3V3DataMgr().rewardSuitDic
  if not careerSuitDic then
    return
  end
  local suitData = careerSuitDic[RoleManager.me and RoleUtility.GetBasicCareer(RoleManager.me.career)]
  if not suitData then
    return
  end
  local equip = suitData[data.suitType]
  if not equip then
    return
  end
  local modelData = QuickFind:GetTeam3V3DataMgr():GetRoleModelShowInfo(equip, self.role_model.transform)
