Instance_DemonPlazaUI = class(BaseUI)
Instance_DemonPlazaUI.layer = UILayer.Panel
Instance_DemonPlazaUI.orderInLayer = 0
Instance_DemonPlazaUI.hideType = UIHideType.WaitDestroy
Instance_DemonPlazaUI.hideFunc = UIHideFunc.MoveOutOfScreen
Instance_DemonPlazaUI.escClose = UIEscClose.DontClose

function Instance_DemonPlazaUI:InitControls()
  self.btn_close = self:GetControl("btn_close")
  self.btn_closeBg = self:GetControl("btn_closeBg")
  self.lab_level = self:GetControl("Middel/level/lab_level")
  self.lab_def = self:GetControl("Middel/def/lab_def")
  self.lab_cond_tips = self:GetControl("Middel/tx_condition/lab_cond_tips")
  ...
  self.DLevelContent = self:GetControl("ScrollviewLevel/Viewport/DLevelContent")
  self.levelBtnItem = self:GetControl("ScrollviewLevel/Viewport/DLevelContent/levelBtnItem")
  self.descBtn = self:GetControl("descBtn")
  self.btn_explain = self:GetControl("btn_explain")
end

function Instance_DemonPlazaUI:OnPreLoad()
end

function Instance_DemonPlazaUI:Init()
...
function Instance_DemonPlazaUI:OnCreate()
  self:InitControls()
  self:InitUI()
  self:RegistUIEvents()
end

function Instance_DemonPlazaUI:InitUI()
  self:InitContent()
end

function Instance_DemonPlazaUI:OnShow()
  self:RegistEvents()
  self:Refresh()
end

function Instance_DemonPlazaUI:OnHide()
...
function Instance_DemonPlazaUI:OnDestroy()
end

function Instance_DemonPlazaUI:RegistUIEvents()
  ...
  self.btn_explain:SetOnClick(self, self.btn_explainOnClick)
  self:SetBtn_get2Click()
end

function Instance_DemonPlazaUI:SetBtn_get2Click()
  local itemData = ItemUtility.GenerateItemData(8000040)
...
function Instance_DemonPlazaUI:btn_closeOnClick(control)
...
function Instance_DemonPlazaUI:btn_get2OnClick(control)
  local temp = {}
  UIManager.Show(UIID.ItemTipUI, {
    item = temp.itemData,
    rightOperate = EItemOperateType.Show,
...
function Instance_DemonPlazaUI:btn_enterOnClick(control)
...
function Instance_DemonPlazaUI:enterDemonPlaza()
  local mapData = {
...
function Instance_DemonPlazaUI:btn_explainOnClick(control)
  local lvCfg = ConfigManager.FindConfigs("cfg_Ui_description", "uiName", "Instance_DemonPlazaUI")
  UIManager.Show(UIID.System_DescUI, {
    id = lvCfg[1].id
  })
end

function Instance_DemonPlazaUI:btn_teamOnClick(control)
  if not InterfaceInforCheck() then
    return
  end
  local prompTipArgs = {
...
function Instance_DemonPlazaUI:RegistEvents()
  self:RegistEvent(Event.Bag_ResBagChange, self.OnResSaleChange, self)
end

function Instance_DemonPlazaUI:Refresh()
  self.meData = ViewData.meData
  self:SetLevelPanel()
  self.enterConditionData, self.LevelAndNumber = TranScriptData.GetEnterConditionData(TranScriptData.TranScriptSubType.DemonPlaza, 100408, 100702)
  ...
  if self.intervalTime <= 0 or RefreshData:CheckIsFirstEnter(4020202) or TaskData.GetTaskById(taskID) and TaskData.GetTaskById(taskID):GetState() == TaskStateType.Accept then
    self.lab_cond_tips:SetText(string.GetColorText("\196\144\195\163 m\225\187\159", "#1ADD1F"))
    self.isSatisfy = true
  else
    self:ShowTimer(self.intervalTime, self.lab_cond_tips)
...
function Instance_DemonPlazaUI:ShowTimer(surplusTime, lab_countdown)
...
function Instance_DemonPlazaUI:InitContent()
...
local str = "Qu\225\186\163ng tr\198\176\225\187\157ng th\225\187\169 %d\n %s"

function Instance_DemonPlazaUI:SetLevelPanel()
  local num = 0
  self.levelTab = TranScriptData.GetLevelTblData(100408)
  local posIndex
  local totalCount = 0
  for i = 1, table.count(self.levelTab) do
    ...
    if transTable.type == TranScriptData.TranScriptSubType.DemonPlaza then
      num = num + 1
      do
  ...
  self.DLevelContent:SetSizeDelta(0, height * totalCount + 400)
  if posIndex then
    self.DLevelContent.transform.anchoredPosition = Vector2(0, height * (posIndex - 2))
  end
end

function Instance_DemonPlazaUI:SetButtonState()
...
function Instance_DemonPlazaUI:lab_levelOnClick(Obj, level)
  ...
  self.enterConditionData, self.LevelAndNumber = TranScriptData.GetEnterConditionData(TranScriptData.TranScriptSubType.DemonPlaza, 100408, 100702, level[1])
  self.ContentData = TranScriptData.GetContentData()
  self:SetPanel()
  self:SetButtonPitchOn(self.LevelTab, Obj)
end

function Instance_DemonPlazaUI:SetButtonPitchOn(ObjTab, Control)
...
function Instance_DemonPlazaUI:SetPanel()
  ...
  local globalId = ClientTable.cfg_Global_globalManager:TryGetValue(60000002).effect
  ...
  local isDef, needDef = TranScriptData.GetDefIsSatisfy(self.enterConditionData.groupId, TranScriptData.TranScriptSubType.DemonPlaza)
  if isDef then
...
function Instance_DemonPlazaUI:OnResSaleChange()
  self:SetPanel()
end
