...
message RoundPlayer {
  ...
  repeated int32 skills = 28;//技能
    int64 horseHp = 29;//坐骑血量
    int64 horseMaxHp = 30;//坐骑最大血量
}
...
message RuWeiInfo{
    ...
    int32 rank = 3;
    int64 unionId = 4;
	bool initUnion = 5;//true:攻城战获胜带入进来的初始战盟
}
...
//四方阵营-单个阵营报名数据
message SiFangZhengBaRuWeiInfo {
    int32 serverId = 1;//自动报名战盟区服id(同区服不可报名)
    repeated RuWeiInfo ruWeiInfo = 2;//已经报名的区服信息
	int32 campId = 3;//阵营id
}

//报名数据
message ResUnionSiFangZhengBaInfo {
    repeated SiFangZhengBaRuWeiInfo info = 1;
	int32 finalWinCamp = 2;//四方争霸最终获胜方阵营id
}

//请求报名加入四方争霸阵营
message ReqUnionSiFangZhengBaBaoMing {
    int32 campId = 1;//SiFangCamp枚举对象
}
...
//跨服四方争霸
message ResSystemInstance_SiFangZhengBaActivity {
	BasicInstance basic = 1;//副本基础数据
	int32 stage = 2;//副本阶段SiFangStage
	int32 campId = 3;//当前正在占领的SiFangCamp
	int32 campScore = 4;//当前阵营的占领分
	int32 winCamp = 5;//占领分达到500或者时间结束时的阵营SiFangCamp
	map<int32, int32> zhenyingKillCount = 6;//阵营杀人进度
	int64 endTime = 7;//副本结束时间，占领分达到要求直接结束
	int64 startTime = 8;//开始1阶段时间
	repeated SiFangFlag flagList = 9;//旗子信息
	repeated SiFangDoor doorList = 10;//城门数据
}

message SiFangDoor {
	int32 x = 1;
	int32 y = 2;
	int32 configId = 3; //城门配置id
	int32 status = 4; //状态0存活、1被破
	int32 type = 5; //1外城、2内城
}
...
message SiFangFlag {
	...
	int32 y = 2;
	int64 rid = 3;//归属玩家id
	string roleName = 4;//归属玩家名字
	int32 campId = 5;//归属玩家所属阵营
}

//跨服四方争霸阵营积分项
message SiFangZhengBaCampScore{
	int32 rank = 1; //排名
	int64 campId = 2; //阵营id
	int32 score = 3; //阵营积分
	string campLeader = 4;//阵营领袖名字
}

//跨服四方争霸个人积分项
message SiFangZhengBaPlayerScore{
	int32 rank = 1; //排名
	int64 pId = 2; //玩家id
	string name = 3; // 玩家名字
	int32 score = 4; //积分
	int32 career = 5;//职业
	string unionName = 6; // 战盟名字
	int32 serverId = 7; //所在区服
	int32 campId = 8;//阵营id
}

//跨服四方争霸战盟积分项
message SiFangZhengBaUnionScore{
	int32 rank = 1; //排名
	int64 unionId = 2; //战盟id
	string unionName = 3; // 战盟名字
	int32 score = 4; //战盟积分
	int32 campId = 5;// 阵营id
}

//四方争霸排行榜
message ResSiFangZhengBaRank{
	repeated SiFangZhengBaCampScore campList = 1; //阵营排行榜
	repeated SiFangZhengBaUnionScore unionList = 2; //战盟排行榜
	repeated SiFangZhengBaPlayerScore playerList = 3; //个人排行榜
	bool close = 4;//true：标识结束，为最终排行榜显示
}

//获得积分冒泡信息，只用于冒泡提示
message ResSiFangGetScore {
	int32 playerScore = 1;//个人获得积分数量
	int32 campScore = 2;//阵营获得积分数量
}

message ResHorseHp{
	repeated HorseData hpDatas = 1;
}

message HorseData{
	int32 itemId = 1;
	int64 hp = 2;
	int32 nextReliveTime = 3;
	int64 maxHp = 4;
}

message ResMonsterHurtList{
	repeated MonsterHurtListInfo bossInfo = 1;
	int32 hide = 2;
}

message MonsterHurtListInfo{
	int64 mid = 1;//怪物Id
	repeated BossHurt hurts= 2;
    int32 midConfig = 3;//怪物配置Id
}